...
export function propagate(nodes, wires) {
  ...
  wires.forEach(w => {
    ...
    // For custom components that have multiple outputs, use node.outputs array
  ...
  // Multi-pass to handle chains (custom-inside-custom etc.)
  for (let pass = 0; pass < 10; pass++) {
    ...
    nodeMap.forEach(node => {
      ...
      if (newValue !== node.value || JSON.stringify(newOutputs) !== JSON.stringify(node.outputs)) {
        ...
        // Update downstream wire values for next pass
        wires.forEach(w => {
